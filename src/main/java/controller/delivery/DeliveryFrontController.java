...
public class DeliveryFrontController extends HttpServlet 
	...
	protected void doProcess(HttpServletRequest request, 
		...
		if(command.equals("/memPurchaseList.deli")) {
			...
			RequestDispatcher dispatcher = request.getRequestDispatcher("delivery/deliveryList.jsp");
			dispatcher.forward(request, response);
		}else if(command.equals("/deliveryRegist.deli")) {
			request.setAttribute("purchaseNum", request.getParameter("purchaseNum"));
			RequestDispatcher dispatcher = 	request.getRequestDispatcher("delivery/deliveryRegist.jsp");
			dispatcher.forward(request, response);
			...
			action.execute(request);
			RequestDispatcher dispatcher = 	request.getRequestDispatcher("delivery/deliveryModify.jsp");
			dispatcher.forward(request, response);
